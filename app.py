...
def _init_db():
    ...
    if not PERSIST_ENABLED:
        ...
        return
    for candidate in [MEMORY_DB_PATH, DEFAULT_DB, "/tmp/memory.sqlite"]:
        try:
            _ensure_parent_dir(candidate)
            conn = _db_conn(candidate)
            cur = conn.cursor()
            ...
            conn.commit()
            conn.close()
            MEMORY_DB_PATH = candidate
            print(f"[memory] using sqlite at {candidate}")
            return
...
def _mars_call(messages: List[Dict[str, str]], conversation_id: str) -> str:
    ...
    if MARS_MIN_TOKENS > 0: payload["min_tokens"] = MARS_MIN_TOKENS
    if MARS_TOP_K > 0: payload["top_k"] = MARS_TOP_K
    if MARS_FREQUENCY_PENALTY: payload["frequency_penalty"] = MARS_FREQUENCY_PENALTY
    if MARS_PRESENCE_PENALTY: payload["presence_penalty"] = MARS_PRESENCE_PENALTY
    if MARS_STOP:
        try:
